...
static void
report_mmas (Xml &xml, SPWAW_SNAPSHOT * /*sp*/, char *label, SPWAW_IMMAS *ptr, SPWAW_SNAP_OOB_FORCE *fp)
{
	if (!ptr || !fp) return;
...
static void
report_mmas (Xml &xml, SPWAW_SNAPSHOT * /*sp*/, char *label, SPWAW_FMMAS *ptr, SPWAW_SNAP_OOB_FORCE *fp)
{
	if (!ptr || !fp) return;
...
report_attrs (Xml &xml, SPWAW_SNAPSHOT *sp, SPWAW_SNAP_OOB_FORCE *f)
{
	...
	}
	{
		...
		report_mmas (xml, sp, "kills",		&(p->mmas_fkills), f);
		report_mmas (xml, sp, "losses",		&(p->mmas_flosses), f);
		report_mmas (xml, sp, "readiness",	&(p->mmas_fready), f);
	}
	{
		XmlScope scope(xml, "unit_attributes");
		report_mmas (xml, sp, "kills",		&(p->mmas_ukills.all), f);
		report_mmas (xml, sp, "experience",	&(p->mmas_uexp.all), f);
		report_mmas (xml, sp, "readiness",	&(p->mmas_uready.all), f);
	}
}
