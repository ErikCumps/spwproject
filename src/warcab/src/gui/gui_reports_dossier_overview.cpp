...
GuiRptDsrOvr::refresh (bool forced)
{
	...
	if (!p) {
		...
		if (p->type == SPWAW_CAMPAIGN_DOSSIER) {
			d.flag->setPixmap (*RES_flag (p->props.OOB));
		...
		if (p->type == SPWAW_CAMPAIGN_DOSSIER) {
			str.printf ("%s core force consists of %u units in %u formations.",
				SPWAW_oob_people (p->props.OOB), p->props.ucnt, p->props.fcnt);
		} else {
		...
		if (p->type == SPWAW_CAMPAIGN_DOSSIER) {
			if (p->bcnt) {
				str.printf ("<pre>");
				str.printf ("<h3>%s campaign losses:</h3>", SPWAW_oob_people (p->props.OOB));
