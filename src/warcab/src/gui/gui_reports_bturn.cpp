...
GuiRptTrn::set_enabled (bool flag)
{
	...
	if (flag) {
		...
		if (d.item->dossier_type != SPWAW_STDALONE_DOSSIER) {
			...
			if (d.item->data.t->snap->OOBp1.support.formations.cnt != 0) addTab (d.force_spt, "Support force");
		} else {
...
GuiRptTrn::refresh (bool forced)
{
	...
	if (d.enabled) {
		...
		if (d.item->dossier_type != SPWAW_STDALONE_DOSSIER) {
			setTabIcon (indexOf (d.force_core), *RES_flagbyid (d.item->data.t->battle->snap->game.battle.strings.flagid_p1));
			setTabIcon (indexOf (d.force_spt), *RES_flagbyid (d.item->data.t->battle->snap->game.battle.strings.flagid_p1));
		} else {
			snprintf (buf, sizeof (buf) - 1, "%s force", d.item->data.t->snap->game.battle.strings.people_p1);
			setTabText (indexOf (d.force_spt), buf);
			setTabIcon (indexOf (d.force_spt), *RES_flagbyid (d.item->data.t->battle->snap->game.battle.strings.flagid_p1));
		}
		snprintf (buf, sizeof (buf) - 1, "%s force", d.item->data.t->snap->game.battle.strings.people_p2);
		setTabText (indexOf (d.force_opp), buf);
		setTabIcon (indexOf (d.force_opp), *RES_flagbyid (d.item->data.t->battle->snap->game.battle.strings.flagid_p2));
	}
