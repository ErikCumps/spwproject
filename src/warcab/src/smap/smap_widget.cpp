...
SmapWidget::load (SPWAW_BTURN *turn)
{
	...
	if (udata->cnt) {
		...
		for (i=0; i<udata->cnt; i++) {
			...
			d.b_info[d.b_cnt].unit		= udata->list[i].data.dname; // FIXME: designation, actually
	...
	if (udata->cnt) {
		...
		for (i=0; i<udata->cnt; i++) {
			...
			d.r_info[d.r_cnt].unit		= udata->list[i].data.dname; // FIXME: designation, actually
...
SmapWidget::setcursor (SmapHexPos &pos)
{
	...
	for (i=0; i<d.b_cnt; i++) {
		...
		data.row[idx].data[UTC_CONTACT]	= d.b_info[i].contact;
		idx++;
	}
	for (i=0; i<d.r_cnt; i++) {
