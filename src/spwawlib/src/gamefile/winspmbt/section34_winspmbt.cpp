...
static SPWAW_ERROR
add_leader (WINSPMBT_LEADER *src, USHORT id, SPWAW_SNAP_OOB_LELRAW *dst, USHORT *idx, STRTAB *stab)
{
	SPWAW_SNAP_OOB_LELRAW	*ptr;

	/* winSPMBT adds a trailing '\n' to the leader's name: remove it here */
	int i = SPWAW_AZSNAME;
	while (i-- > 0) {
		if (src->name[i] == '\n') {
			src->name[i] = '\0';
...
SPWAW_ERROR
section34_winspmbt_save_snapshot (GAMEDATA *src, SPWAW_SNAPSHOT *dst, STRTAB *stab, FULIST &ful1, FULIST &ful2)
{
	SPWAW_ERROR	rc;
	WINSPMBT_LEADER	*data;
	USHORT		idx;
	UEL		*p;

	CNULLARG (src); CNULLARG (dst);

	data = GDWINSPMBT(src)->sec34.u.d.leaders;
