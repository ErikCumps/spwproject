...
//	+ units with a formation sub-ID >= 50 seem to be crews (not so for winSPMBT)
//	+ units have been known to get their own, special, formation,
//	  for which no OOB record ID exists (it can not exist)
//	+ units with a formation sub-ID >= 60 seem to be special attached units (not so for winSPMBT)
//	+ units with a formation sub-ID >= 110 seem to be crews of special attached units (not so for winSPMBT)
//
...
static inline bool
is_this_a_UNIT (WINSPMBT_UNIT *data, USHORT idx)
{
	// winSPMBT sometimes records crews with a regular unit subformation sub-ID?
...
static inline bool
is_this_a_SPAU (WINSPMBT_UNIT *data, USHORT idx)
{
...
static SPWAW_ERROR
find_candidate_units (WINSPMBT_UNIT *udata, WINSPMBT_UNIT_POS *pdata, BYTE player, FULIST &ful)
{
	USHORT	i;
	...
	for (i=0; i<WINSPMBT_UNITCOUNT; i++)
	{
		...
		if (is_this_a_UNIT (udata, i) || is_this_a_SPAU (udata, i))
		{
			...
			if (winspmbt_handling_options.VALIDITY) {
				// Is the unit valid?
				if (!udata[i].valid) {
...
static SPWAW_ERROR
link_candidate_crews (FULIST &ful, WINSPMBT_UNIT *data)
{
	UEL	*p;
	UEL	*uel;
...
find_formation_oobrids (FULIST &ful, SPWOOB *OOB, SPWAW_DATE &date)
{
	...
	while (p)
	{
		...
		if (!ldru && winspmbt_handling_options.SUBFUAL) {
...
verify_candidate_units (FULIST &ful, bool campaign)
{
	...
	while (p)
	{
		...
		if (!winspmbt_handling_options.VALIDITY) {
			...
			if (campaign && winspmbt_handling_options.AUTOCMPCR) {
			...
			if (winspmbt_handling_options.AUTOSPAU) {
...
verify_candidate_formations (FULIST &ful)
{
	...
	while (p)
	{
		...
		if (winspmbt_handling_options.FILTERDUPF) {
		...
		if (winspmbt_handling_options.FILTERDUPF || winspmbt_handling_options.FILTERGAPF) {
			seen[fel->d.FID] = fel;
		}
	...
	if (winspmbt_handling_options.FILTERGAPF) {
...
static SPWAW_ERROR
unitcount (WINSPMBT_UNIT *udata, WINSPMBT_UNIT_POS *pdata, BYTE player, FULIST &ful, SPWOOB *OOB, SPWAW_DATE &date, bool campaign)
{
	SPWAW_ERROR	rc;
...
static SPWAW_ERROR
add_unit (WINSPMBT_UNIT *src, UEL *p, SPWAW_SNAP_OOB_UELRAW *dst, USHORT *idx, STRTAB *stab, SPWOOB *OOB)
{
	SPWAW_ERROR		rc;
	...
	// winSPMBT does not record smoke ammo.
	ptr->smkammo	= 0;
	ptr->crew	= src->crew;
	ptr->range	= src->range;
	// winSPMBT does not seem to record unit stance.
	ptr->stance_x	= 0xFF;
	ptr->stance_y	= 0xFF;
...
SPWAW_ERROR
section01_winspmbt_detection (GAMEDATA *src, SPWAW_SNAPSHOT *dst, FULIST &ful1, FULIST &ful2)
{
	SPWAW_ERROR		rc;
	WINSPMBT_UNIT		*udata;
	WINSPMBT_UNIT_POS	*pdata;
	SPWAW_DATE		date;

	CNULLARG (src); CNULLARG (dst);

	udata = GDWINSPMBT(src)->sec01.u.d.units;
	pdata = GDWINSPMBT(src)->sec17.u.d.pos;
...
SPWAW_ERROR
section01_winspmbt_save_snapshot (GAMEDATA *src, SPWAW_SNAPSHOT *dst, STRTAB *stab, FULIST &ful1, FULIST &ful2)
{
	SPWAW_ERROR	rc;
	WINSPMBT_UNIT	*data;
	SPWAW_DATE	date;
	...
	data = GDWINSPMBT(src)->sec01.u.d.units;
