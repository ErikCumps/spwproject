...
//	+ valid formations must have a leader (not so for winSPMBT)
...
static SPWAW_ERROR
build_formations_list (WINSPMBT_FORMATION *src, BYTE player, FLIST &fl)
{
	int	seen[FORMCOUNT];
	bool	dupf;
	USHORT	i;
	...
	for (i=0; i<WINSPMBT_FORMCOUNT; i++)
	{
		if (winspmbt_handling_options.SKIPNONAME) {
			if (src[i].name[0] == '\0') {
				// skipped: no name
		...
		if (winspmbt_handling_options.SKIPNOLDR) {
		...
		if (winspmbt_handling_options.SKIPNOHCMD) {
		...
		if (winspmbt_handling_options.ALLOWDUPF) {
		...
		if (!commit_FEL (fl, fel, WINSPMBT_FORMPCOUNT)) {
...
static SPWAW_ERROR
build_formations (WINSPMBT_FORMATION *src, BYTE player, FLIST &fl)
{
	SPWAW_ERROR	rc;
...
static SPWAW_ERROR
add_formation (WINSPMBT_FORMATION *src, FEL *p, SPWAW_SNAP_OOB_FELRAW *dst, STRTAB *stab, SPWOOB_FDATA *oob)
{
	SPWAW_UD		*UD;
...
static SPWAW_ERROR
add_formations (WINSPMBT_FORMATION *src, SPWAW_SNAP_OOB_FRAW *dst, FLIST &fl, STRTAB *stab, SPWOOB *oob)
{
	SPWAW_ERROR	rc;
	FEL		*p;
	USHORT		idx;
...
SPWAW_ERROR
section35_winspmbt_detection (WINSPMBT_SECTION35 *src, FULIST &ful1, FULIST &ful2)
{
	SPWAW_ERROR	rc;
...
SPWAW_ERROR
section35_winspmbt_detection (GAMEDATA *src, FULIST &ful1, FULIST &ful2)
{
	return (section35_winspmbt_detection (&(GDWINSPMBT(src)->sec35), ful1, ful2));
}

static SPWAW_ERROR
sec35_save_formations (WINSPMBT_FORMATION *src, SPWAW_SNAP_OOB_FRAW *dst, STRTAB *stab, SPWOOB *oob, FLIST &fl)
{
	SPWAW_ERROR	rc;
...
SPWAW_ERROR
section35_winspmbt_save_snapshot (GAMEDATA *src, SPWAW_SNAPSHOT *dst, STRTAB *stab, FULIST &ful1, FULIST &ful2)
{
	SPWAW_ERROR	rc;

	CNULLARG (src); CNULLARG (dst); CNULLARG (stab);

	rc = sec35_save_formations (GDWINSPMBT(src)->sec35.u.d.formations, &(dst->raw.OOBp1.formations), stab, dst->oobdat, ful1.fl);
	ROE ("sec35_save_formations(OOBp1)");

	rc = sec35_save_formations (GDWINSPMBT(src)->sec35.u.d.formations, &(dst->raw.OOBp2.formations), stab, dst->oobdat, ful2.fl);
