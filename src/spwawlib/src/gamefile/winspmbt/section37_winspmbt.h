...
#ifndef	SECTION37_WINSPMBT_H
#define	SECTION37_WINSPMBT_H	1

#include "gamefile/winspmbt/defines_winspmbt.h"

typedef struct s_WINSPMBT_VHEX {
	BYTE		x[WINSPMBT_VHEXCOUNT];		/* Victory hexes: x-coordinate					*/
	BYTE		y[WINSPMBT_VHEXCOUNT];		/* Victory hexes: y-coordinate					*/
	BYTE		amount[WINSPMBT_VHEXCOUNT];	/* Victory hexes: amount					*/
	BYTE		owner[WINSPMBT_VHEXCOUNT];	/* Victory hexes: owner	0=player1, 1=player2, 2=neutral		*/
} WINSPMBT_VHEX;

typedef struct s_WINSPMBT_GAME_DATA {
	char		__data00[2];
	...
	WINSPMBT_VHEX	vhex;				/* Battle victory hexes						*/
	char		__data02[2];
	...
	char		__data09[2];
	char		location[WINSPMBT_AZSLOCATION];	/* Battle location/description (ASCIIZ)				*/
...
typedef struct s_WINSPMBT_SECTION37 {
	union u_u {
		char	raw[WINSPMBT_SIZESEC37];
		struct s_d {
