...
static SPWAW_ERROR
add_leader (WINSPWW2_LEADER *src, USHORT id, SPWAW_SNAP_OOB_LELRAW *dst, USHORT *idx, STRTAB *stab)
{
	SPWAW_SNAP_OOB_LELRAW	*ptr;

	/* winSPWW2 adds a trailing '\n' to the leader's name: remove it here */
	int i = SPWAW_AZSNAME;
	while (i-- > 0) {
		if (src->name[i] == '\n') {
			src->name[i] = '\0';
...
SPWAW_ERROR
section34_winspww2_save_snapshot (GAMEDATA *src, SPWAW_SNAPSHOT *dst, STRTAB *stab, FULIST &ful1, FULIST &ful2)
{
	SPWAW_ERROR	rc;
	WINSPWW2_LEADER	*data;
	USHORT		idx;
	UEL		*p;

	CNULLARG (src); CNULLARG (dst);

	data = GDWINSPWW2(src)->sec34.u.d.leaders;
