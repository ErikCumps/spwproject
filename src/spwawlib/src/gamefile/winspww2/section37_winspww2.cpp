...
void
section37_winspww2_prepare (WINSPWW2_SECTION37 *src)
{
...
SPWAW_ERROR
section37_winspww2_save_snapshot (GAMEDATA *src, SPWAW_SNAPSHOT *dst, STRTAB *stab)
{
	WINSPWW2_GAME_DATA	*data;
	...
	section37_winspww2_prepare (&(GDWINSPWW2(src)->sec37));

	data = &(GDWINSPWW2(src)->sec37.u.d.data);
	...
	// winSPWW2 does not record weather, assume it's excellent weather
	bp->weather    = W_BEST;
	...
	for (i=0; i<WINSPWW2_VHEXCOUNT; i++) {
