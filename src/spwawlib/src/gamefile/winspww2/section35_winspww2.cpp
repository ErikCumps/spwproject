...
build_formations_list (WINSPWW2_FORMATION *src, BYTE player, FLIST &fl)
{
	...
	for (i=0; i<WINSPWW2_FORMCOUNT; i++)
	{
		...
		if (!commit_FEL (fl, fel, WINSPWW2_FORMPCOUNT)) {
...
static SPWAW_ERROR
build_formations (WINSPWW2_FORMATION *src, BYTE player, FLIST &fl)
{
	SPWAW_ERROR	rc;
...
static SPWAW_ERROR
add_formation (WINSPWW2_FORMATION *src, FEL *p, SPWAW_SNAP_OOB_FELRAW *dst, STRTAB *stab, SPWOOB_FDATA *oob)
{
	SPWAW_UD		*UD;
...
static SPWAW_ERROR
add_formations (WINSPWW2_FORMATION *src, SPWAW_SNAP_OOB_FRAW *dst, FLIST &fl, STRTAB *stab, SPWOOB *oob)
{
	SPWAW_ERROR	rc;
	FEL		*p;
	USHORT		idx;
...
SPWAW_ERROR
section35_winspww2_detection (WINSPWW2_SECTION35 *src, FULIST &ful1, FULIST &ful2)
{
	SPWAW_ERROR	rc;
...
SPWAW_ERROR
section35_winspww2_detection (GAMEDATA *src, FULIST &ful1, FULIST &ful2)
{
	return (section35_winspww2_detection (&(GDWINSPWW2(src)->sec35), ful1, ful2));
}

static SPWAW_ERROR
sec35_save_formations (WINSPWW2_FORMATION *src, SPWAW_SNAP_OOB_FRAW *dst, STRTAB *stab, SPWOOB *oob, FLIST &fl)
{
	SPWAW_ERROR	rc;
...
SPWAW_ERROR
section35_winspww2_save_snapshot (GAMEDATA *src, SPWAW_SNAPSHOT *dst, STRTAB *stab, FULIST &ful1, FULIST &ful2)
{
	SPWAW_ERROR	rc;

	CNULLARG (src); CNULLARG (dst); CNULLARG (stab);

	rc = sec35_save_formations (GDWINSPWW2(src)->sec35.u.d.formations, &(dst->raw.OOBp1.formations), stab, dst->oobdat, ful1.fl);
	ROE ("sec35_save_formations(OOBp1)");

	rc = sec35_save_formations (GDWINSPWW2(src)->sec35.u.d.formations, &(dst->raw.OOBp2.formations), stab, dst->oobdat, ful2.fl);
