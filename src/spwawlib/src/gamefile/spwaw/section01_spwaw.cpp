...
find_formation_oobrids (FULIST &ful, SPWOOB *OOB, SPWAW_DATE &date)
{
	...
	while (p)
	{
		...
		if (!ldru && spwaw_handling_options.SUBFUAL) {
...
verify_candidate_units (FULIST &ful, bool campaign)
{
	...
	while (p)
	{
		...
		if (!spwaw_handling_options.VALIDITY) {
			...
			if (campaign && spwaw_handling_options.AUTOCMPCR) {
...
static SPWAW_ERROR
add_unit (SPWAW_UNIT *src, UEL *p, SPWAW_SNAP_OOB_UELRAW *dst, USHORT *idx, STRTAB *stab, SPWOOB *OOB)
{
	SPWAW_ERROR		rc;
