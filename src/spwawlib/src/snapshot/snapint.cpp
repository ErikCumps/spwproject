...
snapint_oob_units_stage1 (SPWAW_SNAP_OOB_RAW *raw, SPWAW_SNAP_OOB *ptr, SPWOOB_DATA *oob, STRTAB * /*stab*/)
{
	...
	for (i=0; i<(int)raw->units.cnt; i++) {
		...
		// disabled for now, until we find a nicer way of reporting the unit designation
		//dat->tname	 = oob->udata[src->OOBrid].name[0]!='\0' ? strdup(oob->udata[src->OOBrid].name) : src->name;
		dat->tname       = src->name;
